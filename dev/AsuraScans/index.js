...
Type: ${row["type"]}`
  ...
  async function getFilter(source, filter4) {
    const genre = await source.stateManager.retrieve(filter4.toUpperCase()) ?? "";
  ...
  // src/AsuraScans/AsuraScansParser.ts
  var parseMangaDetails = async (source, $2, mangaId) => {
  ...
  var parseChapters = ($2, mangaId) => {
    const chapters = [];
    let sortingIndex = 0;
    for (const chapter of $2("div", "div.pl-4.pr-2.pb-4.overflow-y-auto").toArray()) {
  ...
  var parseChapterDetails = async ($2, mangaId, chapterId) => {
    const pages = [];
    for (const image of $2('img[alt*="chapter"]').toArray()) {
      const img = $2(image).attr("src") ?? "";
      if (!img)
        continue;
      pages.push(img);
    }
  ...
  var parseViewMore = async (source, $2) => {
    const manga = [];
    const collectedIds = [];
  ...
  var parseTags = (filters2) => {
    ...
    const tagSections = [
      ...
      App.createTagSection({
        id: "0",
        label: "genres",
        tags: createTags(filters2.genres, "genres").map((x) => App.createTag(x))
      }),
      // Tag section for status
      App.createTagSection({
        id: "1",
        label: "status",
        tags: createTags(filters2.statuses, "status").map((x) => App.createTag(x))
      }),
      ...
      App.createTagSection({
        id: "3",
        label: "order",
        tags: createTags(filters2.order.map((order) => ({ id: order.value, name: order.name })), "order").map((x) => App.createTag(x))
      })
    ];
    return tagSections;
  };
  var parseSearch = async (source, $2) => {
    const collectedIds = [];
    const itemArray = [];
    for (const item of $2("a", "div.grid.grid-cols-2").toArray()) {
      const slug = $2(item).attr("href")?.replace(/\/$/, "")?.split("/").pop() ?? "";
  ...
  var AsuraScansInfo = {
    ...
    icon: "icon.png",
    contentRating: import__2.ContentRating.MATURE,
  ...
  var AsuraScans = class _AsuraScans extends import__2.Source {
    constructor() {
      super(...arguments);
      // ----REQUEST MANAGER----
    ...
    async getMangaDetails(mangaId) {
      ...
      return await parseMangaDetails(this, $2, mangaId);
    }
    ...
    async getHomePageSections(sectionCallback) {
      ...
      this.CloudFlareError(response.status);
      const $2 = load(response.data);
      await parseHomeSections(this, $2, sectionCallback);
    }
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      this.CloudFlareError(response.status);
      const $2 = load(response.data);
      const manga = await parseViewMore(this, $2);
    ...
    async getSearchResults(query, metadata) {
      ...
      const response = await this.requestManager.schedule(request, 1);
      const $2 = load(response.data);
      const items = await parseSearch(this, $2);
      metadata = !isLastPage($2) ? { page: page + 1 } : void 0;
      return App.createPagedResults({
        results: items,
        metadata
...
Please go to the homepage of <${_AsuraScans.name}> and press the cloud icon.`);
  ...
  var AsuraScans2 = (0, import__3.CompatWrapper)(
    { registerHomeSectionsInInitialise: true },
    new AsuraScans(void 0)
  );
